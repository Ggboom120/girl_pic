...
async def parse_album(url: str, save_folder: Path) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(PICTURE_GATHER, json=requests_body, headers=header) as response:
...
async def get_albums(name: str) -> None:
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(PICTURE_SEARCH, json=requests_body, headers=HEADER) as response:
            html_content = await response.text()
...
async def main() -> None:
    await asyncio.wait([asyncio.create_task(get_albums(name)) for name in FIXED_NAMES])
    logger.info("done")
